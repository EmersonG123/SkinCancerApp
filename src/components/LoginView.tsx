...
export default function LoginView({ onLoginSuccess, onToggleToRegister }: LoginViewProps) {
  const [email, setEmail] = useState("a.ruiz.derm@hospital-central.com");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      // Check if user credentials match any registered user, or just retrieve/mock default
      const usersRaw = localStorage.getItem("medai_users");
      ...
      if (foundUser) {
      ...
      } else if (email === "a.ruiz.derm@hospital-central.com") {
        // Log in default doctor Alejandro Ruiz
        ...
        onLoginSuccess(defaultDoc);
      } else {
        // Create a user on the fly if testing with custom email for easy demo
        const demoDoc: User = {
          name: "Dr. " + email.split("@")[0].charAt(0).toUpperCase() + email.split("@")[0].slice(1),
          email: email,
          license: "CO-" + Math.floor(Math.random() * 90000 + 10000) + "-MED",
          specialty: "Dermatología General",
          analyses: 10,
          precision: 96
        };
        onLoginSuccess(demoDoc);
      }
    }, 1200);
  };
