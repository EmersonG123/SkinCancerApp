...
export default function AnalysisTab({ onAddNewRecord, userAnalysesCount, onUpdateAnalysesCount, selectedRecord, userRol }: AnalysisTabProps) {
  ...
  const [diagnosis, setDiagnosis] = useState<DiagnosticRecord>({
    ...
    nivel_riesgo: "bajo",
    confianza: 98.4,
    explicacion: "Mácula simétrica circular, uniforme de contornos nítidos y coloración homogénea marrón clara.",
    recomendacion: [
    ...
    aviso_legal: "AVISO LEGAL: Esta herramienta utiliza IA como apoyo al diagnóstico clínico. No sustituye el juicio de un profesional médico. Los resultados deben ser validados por un dermatólogo colegiado.",
  ...
  // Load chat messages when diagnosis changes
  useEffect(() => {
    const loadChatHistory = async () => {
      // Si el id es el por defecto, no cargamos historial de BD
      if (diagnosis.id.startsWith("AI-98234") || diagnosis.id === "0") {
        setChatMessages([
          {
            id: "greet-" + Date.now(),
            sender: "assistant",
            text: `Hola, soy el asistente IA de MedAI. He analizado la imagen y detecté indicios de un posible **${diagnosis.nombre_amigable}** con un **${diagnosis.confianza}% de confianza** (nivel de riesgo: **${diagnosis.nivel_riesgo.toUpperCase()}**).\n\n¿Deseas debatir sobre este hallazgo, profundizar en el reporte analítico o consultar directrices de derivación terapéutica?`,
      ...
      try {
        const history = await api.getChatHistorial(diagnosis.id);
        const mapped = history.map((m: any) => ({
          id: String(m.id_mensaje),
          sender: m.rol === "usuario" ? "user" : "assistant",
          text: m.contenido,
  ...
  // Load record selected from history
  useEffect(() => {
    if (selectedRecord) {
      setDiagnosis(selectedRecord);
      setSelectedImage(selectedRecord.imagen);
      setFileName("lesion_historial.jpg");
    }
  }, [selectedRecord]);
  ...
  useEffect(() => {
    if (diagnosis.nivel_riesgo === "alto") {
      ...
      setSuggestionChips([
        "¿La Queratosis Actínica puede malignizar?",
  ...
  }, [diagnosis.nivel_riesgo]);

  // Handle image upload and trigger real backend analysis
  const processImageFile = async (file: File) => {
    if (!file.type.startsWith("image/")) {
      alert("Por favor, seleccione un archivo con formato de imagen válido (JPG, PNG, WEBP, BMP).");
      return;
    }

    setFileName(file.name);
    
    // Crear previsualización local temporal
    ...
    try {
      // Llamar al backend real para analizar
      const result = await api.analizarImagen(file);
      
      clearInterval(progressInterval);
      setUploadProgress(100);

      const formattedRecord: DiagnosticRecord = {
        id: String(result.id_analisis),
        nombre_amigable: result.nombre_amigable,
        clave: result.clase_predicha,
        codigo_icd10: result.clase_predicha === "mel" ? "C43.9" : result.clase_predicha === "nv" ? "D22.9" : "L82.0",
        nivel_riesgo: result.nivel_riesgo,
        confianza: parseFloat(result.confianza),
        explicacion: result.explicacion,
        recomendacion: (result.recomendacion || result.explicacion).split("\n"),
        aviso_legal: result.aviso_legal,
        fecha: new Date(result.fecha_analisis).toLocaleDateString("es-ES", {
          day: "numeric",
          month: "short",
          year: "numeric"
        }),
        imagen: result.imagen.url_imagen
      };

      setTimeout(() => {
        setDiagnosis(formattedRecord);
        setSelectedImage(formattedRecord.imagen);
        onAddNewRecord(formattedRecord);
  ...
  const handleSendMessage = async (customText?: string) => {
    ...
    try {
      ...
      const assistantMsgData = await api.preguntarChat(diagnosis.id, textToSend);
      
      setIsTyping(false);
      const assistantMsg: ChatMessage = {
        id: String(assistantMsgData.id_mensaje),
        sender: "assistant",
        text: assistantMsgData.contenido,
        timestamp: new Date(assistantMsgData.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
      };
      setChatMessages((prev) => [...prev, assistantMsg]);

    } catch (chatError: any) {
      console.error("Error al consultar chatbot:", chatError);
      setIsTyping(false);
      
      const errMsg: ChatMessage = {
        id: "error-" + Date.now(),
        sender: "assistant",
        text: `Error de comunicación: ${chatError.message}. Intente nuevamente.`,
        timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
      };
